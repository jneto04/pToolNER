...
class PortugueseToolNER:
	...
	def loadCorpusInPlainFormat(self, inputFilePath, withNamedEntities, **kwargs):
		try:
			self.sentencesPlain = []
			sentences: list[str] = open(
		...
		if withNamedEntities == True:
			acceptableLabels = kwargs.get('acceptableLabels')
			listTokenTag = []
			self.sentencesPlainWithEntities = []
			for sentence in self.sentencesPlain:
				sentence = sentence.strip()
				tokens = sentence.split(' ')
				for i in range(len(tokens)):
					if i+1 < len(tokens):
						if tokens[i+1] in acceptableLabels:
							tag = tokens[i+1]
							replacedTag = tag.replace('<', '')
							replacedTag = replacedTag.replace('>', '')
							listTokenTag.append(tokens[i]+' '+replacedTag)
						else:
							if tokens[i] not in acceptableLabels:
								listTokenTag.append(tokens[i]+' O')
				self.sentencesPlainWithEntities.append(listTokenTag)
				listTokenTag = []
			self.sentencesPlainWithEntities = [l for l in self.sentencesPlainWithEntities if len(l)>0 ]
